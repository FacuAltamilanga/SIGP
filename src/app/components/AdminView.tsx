...
export function AdminView() {
  ...
  const [booking, setBooking] = useState({ nombre: '', apellido: '', dni: '', fecha_nacimiento: '', sexo: 'no_informado', fecha: today, hora: '', motivo: 'Consulta pediátrica' });
  ...
  async function createAppointment(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(`${getApiUrl()}/turnos`, { method: 'POST', headers, body: JSON.stringify({ fecha: booking.fecha, hora: booking.hora, paciente: { nombre: booking.nombre, apellido: booking.apellido, dni: booking.dni, fecha_nacimiento: booking.fecha_nacimiento, sexo: booking.sexo }, motivo: booking.motivo }) });
      ...
      setShowBooking(false); setBooking({ nombre: '', apellido: '', dni: '', fecha_nacimiento: '', sexo: 'no_informado', fecha: selectedDate, hora: '', motivo: 'Consulta pediátrica' });
