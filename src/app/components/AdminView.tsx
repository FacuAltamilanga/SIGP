...
export function AdminView() {
  ...
  const [booking, setBooking] = useState({ nombre: '', apellido: '', dni: '', fecha_nacimiento: '', sexo: 'no_informado', tutor: '', fecha: today, hora: '', motivo: 'Consulta pediátrica' });
  ...
  async function createAppointment(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(`${getApiUrl()}/turnos`, { method: 'POST', headers, body: JSON.stringify({ fecha: booking.fecha, hora: booking.hora, dni: booking.dni, nombre_paciente: booking.nombre, apellido_paciente: booking.apellido, fecha_nacimiento: booking.fecha_nacimiento, sexo: booking.sexo, paciente: { nombre: booking.nombre, apellido: booking.apellido, dni: booking.dni, fecha_nacimiento: booking.fecha_nacimiento, sexo: booking.sexo }, tutor: booking.tutor, motivo: booking.motivo }) });
      ...
      setShowBooking(false); setBooking({ nombre: '', apellido: '', dni: '', fecha_nacimiento: '', sexo: 'no_informado', tutor: '', fecha: selectedDate, hora: '', motivo: 'Consulta pediátrica' });
